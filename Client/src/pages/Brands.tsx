            ...
            <tbody>
              {allBrand.map((brand) => (
                <tr key={brand.id}>
                  ...
                  <td><span className="brand-slug">{brand.slug}</span></td>
                  <td><span className={`brand-status ${brand.status === "active" ? "active" : "inactive"}`}>{brand.status}</span></td>
                  <td><span>{new Date(brand.createdAt).toLocaleDateString()}</span></td>
                  <td>
                    <div className="action-buttons">
                      <button className="action-btn action-btn-edit" onClick={() => handleEdit(brand)}>
                        <Edit size={16} />
                      </button>
                      <button className="action-btn action-btn-delete" onClick={() => handleDelete(brand.id)}>
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
      ...
      >
        <form onSubmit={handleSubmit} className="brand-form">
          ...
          <div className="form-actions">
            <Button type="button" variant="outline" onClick={() => { setIsAddModalOpen(false); setIsEditModalOpen(false); resetForm(); }}>Cancel</Button>
